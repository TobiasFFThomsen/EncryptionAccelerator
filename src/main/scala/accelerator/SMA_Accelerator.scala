...
class SMA_Accelerator() extends AcceleratorDevice(){
  val sma = Module(new SMA())
	
	val maxBitWidth = 2048
  // States
  ...
  sma.io.b := Cat(bReg.io.out)
  sma.io.t := Cat(tReg.io.out)
  sma.io.n := Cat(nReg.io.out)
  sma.io.valid_in := false.B
  val result = RegInit(0.U(2048.W))

	// running states
	val run_idle :: run_with_load :: run_without_load :: nil_run = Enum(3)

  // Control signals
  val running_state = RegInit(run_idle)
  ...
  when(state_reg === state_idle){
    ...
    when(sma.io.valid_out){
      result := sma.io.result
  ...
  when (master_reg.Cmd === OcpCmd.RD) {
    ...
    }.elsewhen(master_reg.Addr(8, 0) === 300.U) { // 60
      data := result(1951, 1920)
    }.elsewhen(master_reg.Addr(8, 0) === 304.U) {
      data := result(1983, 1952)
    }.elsewhen(master_reg.Addr(8, 0) === 308.U) {
      data := result(2015, 1984)
    }.elsewhen(master_reg.Addr(8, 0) === 312.U) {
...
object GenerateAcceleratorVerilog extends App {
  println("Generating the Verilog file for SMA Accelerator!")
  (new chisel3.stage.ChiselStage).emitVerilog(new SMA_Accelerator())
}
