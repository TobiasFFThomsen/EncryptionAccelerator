...
class AcceleratorDevice() extends Module() {
	
	override val io = new AcceleratorIO()
}

class AcceleratorIO() extends InternalIO() {
	
    val ocp = new OcpCoreSlavePort(ADDR_WIDTH, DATA_WIDTH)
    val ocpBurst = new OcpBurstMasterPort(EXTMEM_ADDR_WIDTH, DATA_WIDTH, BURST_LENGTH)
}
