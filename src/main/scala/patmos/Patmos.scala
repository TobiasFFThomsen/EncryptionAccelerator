...
class Patmos(configFile: String, binFile: String, datFile: String) extends Module {
  ...
  // Make SMA accelerator
  val SMA_accelerator = Module(new SMA_Accelerator())
  
  registerPins("sma_accelerator", SMA_accelerator.io)
  ...
  for (i <- (0 until nrCores)) {
     ...
     (config.Devs
      ...
      .map{case (conf,dev) => 
      {
        ...
        val name = "mmu"
          ...
          val off = 17
          val io = SMA_accelerator.io
          val name = "sma_accelerator"
        } ).filter(e => e != null))
      .map(e => {
        new {
    ...
    val getSlavePort = (_io: Bundle) =>
      _io match {
        ...
        case __io: AcceleratorIO => __io.ocp
      }
	...
	val memarbiter = //Module(new ocp.TdmArbiter(2, ADDR_WIDTH, DATA_WIDTH, BURST_LENGTH)) //Module(new ocp.Arbiter(2, ADDR_WIDTH, DATA_WIDTH, BURST_LENGTH))
      if(ramCtrl.isInstanceOf[DDR3Bridge] || ramCtrl.isInstanceOf[OCRamCtrl]) {
        Module(new ocp.Arbiter(2, ADDR_WIDTH, DATA_WIDTH, BURST_LENGTH))
      } else {
        Module(new ocp.TdmArbiterWrapper(2, ADDR_WIDTH, DATA_WIDTH, BURST_LENGTH))
      }
	
	memarbiter.io.master(0).M <> cores(0).io.memPort.M
	cores(0).io.memPort.S <> memarbiter.io.master(0).S
	
	memarbiter.io.master(1).M <> SMA_accelerator.io.ocpBurst.M
	SMA_accelerator.io.ocpBurst.S <> memarbiter.io.master(1).S
